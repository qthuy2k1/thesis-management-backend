...
func (r *CommiteeRepo) CreateCommiteeUserDetail(ctx context.Context, p CommiteeUserDetailInputRepo) (CommiteeUserDetailOutputRepo, error) {
	row, err := QueryRowSQL(ctx, r.Database, "CreateCommiteeUserDetail", "INSERT INTO thesis_commitee_user_details (commitee_id, lecturer_id, student_id) VALUES ($1, $2, $3) RETURNING commitee_id, lecturer_id, student_id", p.CommiteeID, p.LecturerID, p.StudentID)
	if err != nil {
		return CommiteeUserDetailOutputRepo{}, err
	}

	var commiteeUserDetailOutput CommiteeUserDetailOutputRepo
	if err := row.Scan(&commiteeUserDetailOutput.CommiteeID, &commiteeUserDetailOutput.LecturerID, &commiteeUserDetailOutput.StudentID); err != nil {
		return CommiteeUserDetailOutputRepo{}, err
	}
...
func (r *CommiteeRepo) GetCommiteeUserDetail(ctx context.Context, id int) (CommiteeUserDetailOutputRepo, error) {
	...
	if err = row.Scan(&commitee.CommiteeID, &commitee.LecturerID, &commitee.StudentID); err != nil {
...
func (r *CommiteeRepo) IsCommiteeUserDetailExists(ctx context.Context, commiteeID int, lecturerID string, studentID string) (bool, error) {
	var exists bool
	row, err := QueryRowSQL(ctx, r.Database, "IsCommiteeUserDetailExists", "SELECT EXISTS(SELECT 1 FROM thesis_commitee_user_details WHERE commitee_id=$1 AND lecturer_id=$2 AND student_id=$3)", commiteeID, lecturerID, studentID)
	if err != nil {
		return false, err
	}
...
func (r *CommiteeRepo) UpdateCommiteeUserDetail(ctx context.Context, commitee CommiteeUserDetailInputRepo) error {
	...
	result, err := ExecSQL(ctx, r.Database, "UpdateCommiteeUserDetail", "UPDATE thesis_commitee_user_details SET commitee_id=$2, lecturer_id=$3, student_id=$4 WHERE id=$1", commitee.CommiteeID, commitee.LecturerID, commitee.StudentID)
	if err != nil {
		return err
	}
...
func (r *CommiteeRepo) DeleteCommiteeUserDetail(ctx context.Context, commiteeID int, lecturerID, studentID string) error {
	result, err := ExecSQL(ctx, r.Database, "DeleteCommiteeUserDetail", "DELETE FROM thesis_commitee_user_details WHERE commitee_id=$1, lecturer_id=$2, student_id=$3", commiteeID, lecturerID, studentID)
	if err != nil {
		return err
	}
