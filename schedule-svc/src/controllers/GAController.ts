...
import {
  ...
  ITimeSlotItem,
  IParamAllSchedule
} from "../interface/scheduleDef";
import { NIL, v4 as uuidv4 } from "uuid";
import { IStudentDef } from "../interface/studentDef";
import { INITIATE_STUDENT_DEF } from "../data";
import { ScheduleDefModel } from "../models/scheduledModel";
import { IAuthObject } from "../interface/auth";

export const GAScheduleController = {
  ...
  //   }
  // },
  async scheduleGAThesisDefense(req: any) {
    try {
      const scheduleParams: IParamSchedule = {
  ...
  async getSchedule(req: IParamSchedule): Promise<IThesisDef | undefined> {
    try {
      const schedule = await ScheduleDefModel.getSchedule();
      if (!schedule) {
  ...
  async getCouncilInSchedule(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const schedule = await ScheduleDefModel.getCouncilInSchedule(id);
      if (!schedule) {
  ...
  async getScheduleForStudent(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const schedule = await ScheduleDefModel.getScheduleForStudent(id);
      if (!schedule) {
        res.status(404).json("Schedule is empty");
      } else {
        res.status(200).json(schedule);
  ...
  async getScheduleForLecturer(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const schedule = await ScheduleDefModel.getScheduleForLecturer(id);
      if (!schedule) {
        res.status(404).json("Schedule is empty");
      } else {
        res.status(200).json(schedule);
